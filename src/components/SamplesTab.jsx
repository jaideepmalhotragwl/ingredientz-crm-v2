...
function stageLabel(s) {
  if (s.stage === "Feedback" && s.feedback_result) return s.feedback_result;     // Approved / Rejected
...
export function SamplesTab({ samples, onSelect, onNew }) {
  ...
  const metrics = useMemo(() => {
    const open = samples.filter(s => s.stage !== "Feedback");
    ...
    const awaitingFeedback = samples.filter(s => ["Dispatched to Customer", "Customer Received"].includes(s.stage)).length;
    return { open: open.length, atWarehouse, withCustomer, awaitingFeedback };
  }, [samples]);
  ...
  const filtered = useMemo(() => {
    return samples.filter(s => {
      ...
      if (search) {
        ...
        const hay = `${s.sample_number || ""} ${s.customer_name || ""} ${s.supplier_name || ""} ${s.product_name || ""}`.toLowerCase();
  ...
  const btnPrimary = {
    background: C.blue, color: "white", border: 0, padding: "8px 16px", borderRadius: 8,
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10, marginBottom: 20 }}>
        ...
        <Metric label="Awaiting feedback" value={metrics.awaitingFeedback} />
      </div>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 14, flexWrap: "wrap" }}>
        <input
          ...
          placeholder="Search by sample #, customer, supplier, product..."
          value={search}
        ...
        <button style={btnPrimary} onClick={onNew}>+ New sample request</button>
      </div>
      ...
      <div style={{ ...card, padding: 0, overflow: "hidden" }}>
        {filtered.length === 0 ? (
          <div style={{ padding: 40, textAlign: "center", color: C.muted, fontSize: 13 }}>
            {samples.length === 0
              ? "No samples yet. Click + New sample request to start one."
            ...
            <tbody>
              {filtered.map(s => (
                ...
                >
                  ...
                  <Td style={{ color: C.muted }}>{s.supplier_name || "—"}</Td>
...
function Th({ children }) {
  return (
    <th style={{ textAlign: "left", padding: "10px 14px", fontWeight: 600, fontSize: 11, color: C.muted, textTransform: "uppercase", letterSpacing: 0.5 }}>{children}</th>
  );
}
