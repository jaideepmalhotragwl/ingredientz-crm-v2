...
import { uploadOrderDocument, getOrderDocumentUrl, deleteOrderDocument } from "../lib/orderUtils.js";
...
const CERT_STANDARDS = ["ISO 9001", "ISO 22000", "FSSC 22000", "GMP", "HACCP", "Halal", "Kosher", "Organic", "USP", "EP", "FCC", "BRC", "Kosher/Halal", "Other"];
...
export function SupplierDrawer({ supplier, onClose, onSaved }) {
  ...
  function seedProfile(s) {
    s = s || {};
    return {
      company: s.company || "", contact_name: s.contact_name || "", contact_email: s.contact_email || "",
  ...
  async function loadAll() {
    const [{ data: d }, { data: sp }, { data: ap }] = await Promise.all([
      supabase.from("supplier_documents").select("*").eq("supplier_id", supplier.id).order("uploaded_at", { ascending: false }),
  ...
  async function saveProfile() {
    setSavingProf(true);
    const patch = {
      company: prof.company.trim(), contact_name: prof.contact_name, contact_email: prof.contact_email,
  ...
  async function saveBank() {
    ...
    const { error } = await supabase.from("suppliers").update(patch).eq("id", supplier.id);
    setSavingBank(false);
    if (error) { alert("Could not save bank details."); return; }
    onSaved && onSaved();
  }
  ...
  async function uploadDoc(file, docType, extra = {}) {
    if (!file) return;
    setBusy(true);
    const { path, error } = await uploadOrderDocument(file, `suppliers/${supplier.id}/${docType}`);
    if (error || !path) { setBusy(false); alert("Upload failed."); return; }
    const row = { supplier_id: supplier.id, doc_type: docType, name: file.name, file_url: path, ...extra };
    const { data, error: e2 } = await supabase.from("supplier_documents").insert(row).select().single();
    setBusy(false);
    if (e2) { alert("Saved file but could not record it."); return; }
    setDocs(p => [data, ...p]);
  }
  ...
  async function addProduct(productId) {
    ...
    if (error) { alert("Could not add product."); return; }
  ...
  const otherDocs = docs.filter(d => d.doc_type !== "certificate");
    ...
    <div style={overlay} onClick={onClose}>
      <div style={drawer} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px" }}>
          ...
          {tab === "profile" && (
            <div style={card}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <Field k="contact_email" l="Email" ph="supplier@company.com" type="email" />
          ...
          {tab === "certs" && (
            <div style={card}>
              <AddCert onAdd={(file, standard, expiry) => uploadDoc(file, "certificate", { cert_standard: standard, expiry_date: expiry || null })} busy={busy} C={C} inp={inp} label={label} btnG={btnG} />
              <div style={{ marginTop: 12 }}>
                {certs.length === 0 ? <Empty C={C}>No certificates yet.</Empty> : certs.map(d => (
                  <Row key={d.id} C={C}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: C.ink }}>{d.cert_standard || "Certificate"}</div>
                      <div style={{ fontSize: 11, color: C.muted }}>{d.name}{d.expiry_date ? ` · expires ${d.expiry_date}` : ""}</div>
                    </div>
          ...
          {tab === "docs" && (
            <div style={card}>
              <AddDoc onAdd={(file, docType) => uploadDoc(file, docType)} busy={busy} inp={inp} label={label} btnG={btnG} C={C} />
              <div style={{ marginTop: 12 }}>
                {otherDocs.length === 0 ? <Empty C={C}>No documents yet.</Empty> : otherDocs.map(d => (
                  <Row key={d.id} C={C}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: C.ink }}>{(DOC_TYPES.find(x => x.v === d.doc_type) || {}).l || d.doc_type}</div>
...
function AddCert({ onAdd, busy, inp, label, btnG, C }) {
...
function AddDoc({ onAdd, busy, inp, label, btnG, C }) {
